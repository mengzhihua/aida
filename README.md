...
**第二十九轮** 补齐 softlockup 全 CPU backtrace、`io_delay_type`、extfrag/stat_interval、TCP TFO blackhole/MSS/reordering/recovery、IPv6 enhanced DAD / auto_flowlabels、ICMP msgs 限速，以及 scsi_disk / scsi_tape / graphics leftover class。不 dump `tcp_fastopen_key` / `serial-base`，不读 `cad_pid`。
