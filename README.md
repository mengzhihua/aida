...
**第二十七轮** 补齐 hung/oops 全 CPU backtrace、`compaction_proactiveness`、SCHED_DEADLINE 周期、TCP thin-linear / compressed SACK / fwmark，以及 CXL / devfreq / FPGA / GNSS leftover class。GUI `refresh_live` 会更新运行期 cpuidle governor。不 dump `tcp_fastopen_key` / `serial-base`，不读 `cad_pid`。
...
| 网络 | `/sys/class/net` + getifaddrs；snmp/softnet/TcpExt；IPv6 snmp6/路由/rt6_stats 第 6 列；TCP knobs/rmem/notsent/tcp_mem/udp_mem/orphans/dsack/autocorking/retries1/early_retrans/frto/min_tso/pacing_ss/pacing_ca/orphan_retries/rfc1337/ecn_fallback/abort_overflow/no_metrics/challenge_ack/thin_linear/limit_output/comp_sack/fwmark；tcp6/udp6/raw/udplite/raw6；xfrm_stat；ptype；fib_triestat Leaves；igmp6 按接口去重；iptables 表名；busy_poll/busy_read/dev_weight/rps_sock_flow_entries；IPv6 accept_ra/autoconf/hop/accept_dad/addr_gen_mode/max_addresses/accept_ra_defrtr/router_solicitations/dad_transmits/ndisc_notify；ip6frag/ipfrag/ipfrag_time/ipfrag_max_dist；conntrack 超时/buckets；tcp_max_tw_buckets；icmp_ratelimit；icmp_echo_ignore_all/enable_probe；ip_default_ttl；ip_no_pmtu_disc；ip_nonlocal_bind；ip_dynaddr；ip_forward_update_priority；fib_notify_on_flag_change；fib_multipath_hash_policy；netdev_tstamp_prequeue；message_cost/burst；ip_unprivileged_port_start；bindv6only；protocols；conntrack；net.core；rp_filter/use_tempaddr/accept_dad/addr_gen_mode/accept_ra_defrtr/router_solicitations/dad_transmits/ndisc_notify per-iface |
