...
**第十轮** 补齐内核 sysctl、cgroup v2、TCP/IP snmp、网桥/bond、zoneinfo、IRQ 亲和、ext4 写入量和控制台。
...
| 网络 | `/sys/class/net` + getifaddrs；queues；sockstat；snmp；softnet；bridge/bond |
...
| PSI / IRQ / taint / LSM / sysctl / cgroup | pressure、interrupts+affinity、tainted、LSM、file-nr/pid_max、cgroup v2 |
...
└─────────────────────┘           │
   ...
   topology/idle  /dmi/id      /hwmon      ioctl       class 03    queues
                                  ...
                                  ├── input / audio / power / firmware / EDAC / TPM / sysctl / cgroup
...
3. **GUI 与 CLI 共用同一套 snapshot**，GUI 每 ~0.8s 刷新传感器、告警、网卡/磁盘速率、RAPL 瓦特、内存、zram/zswap、loadavg、clocksource/PTP、EDAC、PSI、IRQ/softirq、挂载用量、平台亮度、sysctl/cgroup 和 `/proc/stat`，不全量重扫 PCI/USB/virtio/KVM/IOMMU/MD/SCSI/iSCSI/模块/iomem/ATA。
