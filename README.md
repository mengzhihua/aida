...
**第二十二轮** 补齐 CPU `possible`/`present`/`kernel_max`、vm dirty/overcommit 字节与 pipe 页数、TCP `tcp_mem`/`udp_mem`、IPv6 DAD/`addr_gen_mode`，以及 remoteproc/extcon/tee/mdio_bus 与 wakeup 源计数。不写 `compact_memory`。

| 模块 | 状态 |
| --- | --- |
| CPU / DMI / hwmon / NVMe / PCI / 块设备 / 软件 | 可读 sysfs/procfs；CPU `offline`/`possible`/`present`/`kernel_max`；`ostype`；firmware device-tree `model` |
...
| PSI / IRQ / taint / LSM / sysctl / cgroup / 安全 | pressure、interrupts、sysfs irq、lockdown/kptr、file-nr、aio/inotify、boot_id、panic/sysrq、keys、SysV IPC、fs.protected、sched_rt/OOM、printk/cfs/uffd、bpf_jit/binfmt_misc、dentry-state、inode-state、pty、io_uring、dirty_bytes/overcommit_kbytes、pipe-user-pages、key-users、cgroup v1 enabled |
| ATA / MD / SCSI / iSCSI | ata_port；mdstat；scsi_host + scsi_device；iscsi_transport（不调用 iscsiadm）；dm name/uuid；BDI/BSG |
| 平台 / 总线 | watchdog/LED/I2C；rfkill/蓝牙/雷电/V4L/MMC/MEI；ttyS；misc；HID；GPIO/MTD/IB；MSR；vtconsole；`bus/platform/devices`；ieee80211/typec/udc/dax/wmi/spi/serio/ubi；scsi_generic/wwan/ppp/phy；remoteproc/extcon/tee/mdio_bus；wakeup 源计数 |
