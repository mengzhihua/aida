...
**第三十三轮** 补齐 NUMA promote 速率、legacy VA、hugetlb shm 组、core note 上限；UDP wmem_min / TCP shrink/l3mdev/migrate/reflect/rto_min；IPv6 RA min hop 与 ICMP inbound ifaddr；Fibre Channel / accel / vDPA leftover。状态栏加上 load 与内存用量。不 dump `netdev_rss_key` / `stable_secret` / `mmap_rnd_bits` / `tcp_fastopen_key` / `cad_pid`。
...
| 平台 / 总线 | watchdog/LED/I2C；rfkill/蓝牙/雷电/V4L/MMC/MEI；ttyS；misc；HID；GPIO/MTD/IB；MSR；vtconsole；`bus/platform/devices`；ieee80211/typec/udc/dax/wmi/spi/serio/ubi；scsi_generic/wwan/ppp/phy；remoteproc/extcon/tee/mdio_bus；spi_master/i2c-dev/nvme-subsystem/w1；macvtap/tun/nvme-generic/nvme-fabrics；iscsi_endpoint/iface/connection/flashnode；bus/container；nd/dma_heap；cxl（bus/devices）/devfreq/fpga_manager/bridge/region/gnss；rpmsg/devcoredump；scsi_disk/scsi_tape/graphics；cec/media；nbd（`class/block`）；vfio（`class/vfio`+`vfio-dev`）/mdev（`bus/mdev`）/vhost（misc+`/dev`）；fc_host/fc_remote_ports/fc_vports；accel；vdpa（bus）；wakeup 源计数 |
