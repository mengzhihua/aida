...
| 模块 | 状态 |
| --- | --- |
| CPU / DMI / hwmon / NVMe / PCI / 块设备 / 软件 | 可读 sysfs/procfs；CPU `offline`/`possible`/`present`/`kernel_max`/`enabled`/`nohz_full`/`modalias`；`ostype`；firmware device-tree `model` |
