...
**第三十轮** 补齐 `printk_delay`、`max_lock_depth`、`perf_event_mlock_kb`、hugetlb vmemmap 优化、TCP max_reordering/TSO divisor/UDP early demux/SYN linear/forward PMTU/no_ssthresh、IPv6 flowlabel/idgen/accept_ra_mtu，以及 cec / media / nbd leftover class。不 dump `mmap_rnd_bits` / `tcp_fastopen_key` / `serial-base`，不读 `cad_pid`。
...
| 平台 / 总线 | watchdog/LED/I2C；rfkill/蓝牙/雷电/V4L/MMC/MEI；ttyS；misc；HID；GPIO/MTD/IB；MSR；vtconsole；`bus/platform/devices`；ieee80211/typec/udc/dax/wmi/spi/serio/ubi；scsi_generic/wwan/ppp/phy；remoteproc/extcon/tee/mdio_bus；spi_master/i2c-dev/nvme-subsystem/w1；macvtap/tun/nvme-generic/nvme-fabrics；iscsi_endpoint/iface/connection/flashnode；bus/container；nd/dma_heap；cxl（bus/devices）/devfreq/fpga_manager/bridge/region/gnss；rpmsg/devcoredump；scsi_disk/scsi_tape/graphics；cec/media/nbd；wakeup 源计数 |
