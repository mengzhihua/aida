...
219. **AppImage / CLI 版本号只维护在 `Cargo.toml`。** `./scripts/package.sh` 读它生成 `AIDA_Linux-<ver>-<arch>.AppImage` 与 `aida-cli-<ver>-<arch>-{musl|gnu}`。linuxdeploy 缓存在 `.cache/`，不要提交。无 FUSE 时必须 `APPIMAGE_EXTRACT_AND_RUN=1`。GUI 不要 musl 静态。
