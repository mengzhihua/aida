...
| DMI | `/sys/class/dmi/id/*` | `/sys/firmware/dmi/tables/DMI` SMBIOS 结构；Type 16 Physical Memory Array（位置 / ECC / 最大容量 / 槽位数）；Type 17 按 DSP0134：速度 `0x15`、类型 `0x12`、外形 `0x0E`、配置速度 `0x20`、位宽/rank、厂商/序列/料号用字符串号（`0`=未用）；空槽 size=0 显示 empty；不调用 dmidecode，不扫 I2C SPD |
