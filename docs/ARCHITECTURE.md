...
| Net | `/sys/class/net/*/statistics` | getifaddrs；queues；sockstat；snmp；softnet；bridge/bond；conntrack count/max/buckets/established timeout；tcp congestion；`/proc/net/netstat`；snmp6；net.core busy_poll/busy_read/dev_weight/rps_sock_flow_entries/netdev_tstamp_prequeue/message_cost/message_burst；ipv6_route；if_inet6；tcp knobs/rmem/notsent_lowat/adv_win_scale/max_tw_buckets/tcp_mem/udp_mem/orphans/dsack/autocorking/retries1/early_retrans/frto/min_tso/pacing_ss/pacing_ca/invalid_ratelimit/orphan_retries/rfc1337/ecn_fallback/abort_overflow/no_metrics/challenge_ack（INT_MAX 不额外收紧）/thin_linear/limit_output/comp_sack/fwmark；tcp6/udp6/raw/udplite/raw6/udplite6；xfrm_stat；ptype（function 取末 token）；fib_triestat 主表 Leaves（不读 fib_trie）；igmp6 按接口名去重；ip_tables_names（空=未加载，denied≠空）；connector 名；protocols；rt6_stats 第 6 列 dst cache；rp_filter/use_tempaddr/accept_dad/addr_gen_mode/accept_ra_defrtr/router_solicitations/dad_transmits/ndisc_notify 含与 `conf/all` 不同的接口；igmp 只计接口头行；IPv6 accept_ra/autoconf/hop_limit/accept_dad/addr_gen_mode/max_addresses；ip6frag high/low；ipfrag high/low/time/max_dist；ip_no_pmtu_disc；fib_multipath_hash_policy；fib_notify_on_flag_change；icmp_ratelimit；icmp_echo_ignore_all/enable_probe；ip_default_ttl；ip_unprivileged_port_start；bindv6only；ip_nonlocal_bind；ip_dynaddr；ip_forward_update_priority；不 dump `tcp_fastopen_key` |
