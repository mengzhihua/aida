...
pub fn to_html(snap: &HardwareSnapshot) -> String {
    ...
    for (label, names) in [
        ...
        ("intel_th", &snap.buses.intel_th),
        ("coresight", &snap.buses.coresight),
        ("ntb", &snap.buses.ntb),
        ("xen", &snap.buses.xen),
    ...
    html.push_str(&format!(
        ...
        match snap.net.ipv6_optimistic_dad.value.as_deref() {
            Some("0") => "0 关".into(),
            Some("1") => "1 乐观".into(),
        ...
        match snap.net.ipv6_disable_xfrm.value.as_deref() {
            Some("0") => "0 开 IPsec".into(),
            Some("1") => "1 关 xfrm".into(),
            _ => snap.net.ipv6_disable_xfrm.display(),
        },
        match snap.net.ipv6_skip_notify_on_dev_down.value.as_deref() {
            Some("0") => "0 通知".into(),
            Some("1") => "1 跳过".into(),
    ...
    if !snap.net.ipv6_optimistic_dad_dev.is_empty() {
        html.push_str(&format!(
            "<p class=\"muted\">optimistic_dad iface {}</p>",
            esc(&snap.net.ipv6_optimistic_dad_dev.join(" "))
        ));
    }
    if !snap.net.ipv6_accept_source_route_dev.is_empty() {
        html.push_str(&format!(
    ...
    if !snap.net.ipv6_disable_xfrm_dev.is_empty() {
        html.push_str(&format!(
            "<p class=\"muted\">disable_xfrm iface {}</p>",
            esc(&snap.net.ipv6_disable_xfrm_dev.join(" "))
        ));
    }
    if !snap.net.ipv6_skip_notify_on_dev_down_dev.is_empty() {
        html.push_str(&format!(
            "<p class=\"muted\">skip_notify_on_dev_down iface {}</p>",
            esc(&snap.net.ipv6_skip_notify_on_dev_down_dev.join(" "))
        ));
    }
    if !snap.net.ipv6_regen_max_retry_dev.is_empty() {
        html.push_str(&format!(
            "<p class=\"muted\">regen_max_retry iface {}</p>",
            esc(&snap.net.ipv6_regen_max_retry_dev.join(" "))
        ));
    }
    if !snap.net.ipv6_max_desync_factor_dev.is_empty() {
        html.push_str(&format!(
...
fn html_dmi_memory(html: &mut String, snap: &HardwareSnapshot) {
    ...
    if !snap.dmi.mapped_addresses.is_empty() {
        html.push_str("<p class=\"muted\">SMBIOS Type 19 内存映射</p>");
        html.push_str("<table><tr><th>范围</th><th>阵列句柄</th><th>宽度</th></tr>");
        for m in &snap.dmi.mapped_addresses {
            html.push_str(&format!(
    ...
    if !snap.dmi.device_mapped.is_empty() {
        html.push_str("<p class=\"muted\">SMBIOS Type 20 设备映射</p>");
        html.push_str("<table><tr><th>范围</th><th>设备句柄</th><th>映射句柄</th><th>行</th><th>交错</th></tr>");
        for m in &snap.dmi.device_mapped {
            html.push_str(&format!(
                "<tr><td>{}</td><td>{:#06X}</td><td>{:#06X}</td><td>{}</td><td>{}/{}</td></tr>",
                crate::probes::dmi::hex_range(m.start, m.end),
                m.device_handle,
                m.mapped_handle,
    ...
    if !snap.dmi.memory_errors.is_empty() {
        html.push_str("<p class=\"muted\">SMBIOS Type 18 内存错误</p>");
        html.push_str("<table><tr><th>类型</th><th>粒度</th><th>操作</th><th>Syndrome</th><th>阵列地址</th><th>设备地址</th><th>分辨率</th></tr>");
        for e in &snap.dmi.memory_errors {
            html.push_str(&format!(
                "<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>",
                esc(&e.error_type),
                esc(&e.granularity),
    ...
    if !snap.dmi.memory_errors64.is_empty() {
        html.push_str("<p class=\"muted\">SMBIOS Type 33 64 位内存错误</p>");
        html.push_str("<table><tr><th>类型</th><th>粒度</th><th>操作</th><th>Syndrome</th><th>阵列地址</th><th>设备地址</th><th>分辨率</th></tr>");
        for e in &snap.dmi.memory_errors64 {
            html.push_str(&format!(
                "<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>",
                esc(&e.error_type),
                esc(&e.granularity),
...
fn html_dmi_board(html: &mut String, snap: &HardwareSnapshot) {
    ...
    if !snap.dmi.ports.is_empty() {
        html.push_str("<p class=\"muted\">SMBIOS Type 8 端口</p>");
        html.push_str("<table><tr><th>内部</th><th>外部</th><th>连接器</th><th>端口</th></tr>");
        for p in &snap.dmi.ports {
            html.push_str(&format!(
                "<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>",
                esc(p.internal.as_deref().unwrap_or("—")),
                esc(p.external.as_deref().unwrap_or("—")),
                esc(p.connector.as_deref().unwrap_or("—")),
                esc(p.port.as_deref().unwrap_or("—"))
            ));
        }
        html.push_str("</table>");
    }
    if !snap.dmi.onboard.is_empty() {
        html.push_str("<p class=\"muted\">SMBIOS Type 41 板载设备</p>");
        html.push_str("<table><tr><th>名称</th><th>类型</th><th>状态</th><th>总线</th></tr>");
        for d in &snap.dmi.onboard {
            html.push_str(&format!(
                "<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>",
                esc(d.designation.as_deref().unwrap_or("—")),
                esc(d.kind.as_deref().unwrap_or("—")),
                if d.enabled { "enabled" } else { "disabled" },
                esc(d.bus.as_deref().unwrap_or("—"))
            ));
        }
        html.push_str("</table>");
    }
    if !snap.dmi.power_supplies.is_empty() {
        html.push_str("<p class=\"muted\">SMBIOS Type 39 电源</p>");
        html.push_str("<table><tr><th>名称</th><th>厂商</th><th>位置</th><th>功率</th><th>状态</th></tr>");
        for p in &snap.dmi.power_supplies {
            html.push_str(&format!(
                "<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>",
                esc(p.name.as_deref().unwrap_or("—")),
                esc(p.manufacturer.as_deref().unwrap_or("—")),
                esc(p.location.as_deref().unwrap_or("—")),
                p.max_watts.map(|n| format!("{n} W")).unwrap_or_else(|| "—".into()),
    ...
    if !snap.dmi.pointing_devices.is_empty() {
        html.push_str("<p class=\"muted\">SMBIOS Type 21 板载指针</p>");
        html.push_str("<table><tr><th>类型</th><th>接口</th><th>按键</th></tr>");
        for p in &snap.dmi.pointing_devices {
            html.push_str(&format!(
                "<tr><td>{}</td><td>{}</td><td>{}</td></tr>",
                esc(&p.kind),
                esc(&p.interface),
    ...
    if let Some(r) = &snap.dmi.remote_access {
        html.push_str(&format!(
            "<p class=\"muted\">SMBIOS Type 30 带外访问 {}  inbound {}  outbound {}</p>",
            esc(r.manufacturer.as_deref().unwrap_or("—")),
...
fn report_sections(snap: &HardwareSnapshot) -> Vec<ReportSection> {
    ...
    for (i, m) in snap.dmi.mapped_addresses.iter().take(8).enumerate() {
        dmi.push(pair(
            format!("映射 {i}"),
            format!(
                "{}  array {:#06X}  width {}",
    ...
    for (i, m) in snap.dmi.device_mapped.iter().take(8).enumerate() {
        dmi.push(pair(
            format!("设备映射 {i}"),
            format!(
                "{}  DIMM {:#06X}  map {:#06X}",
    ...
    for e in snap.dmi.memory_errors.iter().take(8) {
        dmi.push(pair(
            &e.error_type,
            format!(
                "{}  {}  syn {}",
    ...
    for e in snap.dmi.memory_errors64.iter().take(8) {
        dmi.push(pair(
            &e.error_type,
            format!(
                "64-bit  {}  {}  syn {}",
    ...
    for p in snap.dmi.ports.iter().take(16) {
        dmi.push(pair(
            p.external
                .as_deref()
    ...
    for d in snap.dmi.onboard.iter().take(16) {
        dmi.push(pair(
    ...
    for p in snap.dmi.power_supplies.iter().take(8) {
        dmi.push(pair(
    ...
    for p in snap.dmi.pointing_devices.iter().take(4) {
        dmi.push(pair(
            &p.kind,
