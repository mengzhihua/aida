...
impl InputState {
    ...
    ///
    /// Local patch (aida): pointer motion must not schedule a 0-delay frame.
    /// On llvmpipe that follow-up arrives while the next motion event is
    /// already queued, so the window repaints as fast as the CPU allows.
    pub fn wants_repaint_after(&self) -> Option<Duration> {
        if self.unprocessed_scroll_delta.abs().max_elem() > 0.2
