...
//!
//! Local patch (aida): pointer-move repaints are capped so llvmpipe does not
//! redraw the whole window on every motion event. See `pointer_move_should_repaint`.
//!
//! ## Feature flags
...
pub struct State {
    ...
    /// Last time a pointer move was allowed to schedule a redraw.
    last_pointer_repaint: web_time::Instant,
}
...
impl State {
    ...
    pub fn new(
        ...
        let mut slf = Self {
            ...
            allow_ime: false,
            ime_rect_px: None,
            last_pointer_repaint: web_time::Instant::now()
    ...
    pub fn on_window_event(
        ...
        match event {
            ...
            WindowEvent::CursorMoved { position, .. } => {
                ...
                EventResponse {
                    repaint: self.pointer_move_should_repaint(),
            ...
            WindowEvent::Touch(touch) => {
                ...
                let repaint = match touch.phase {
                    winit::event::TouchPhase::Moved => self.pointer_move_should_repaint(),
                    _ => true,
                };
                EventResponse {
                    repaint,
    ...
    fn on_cursor_moved(
        ...
        if self.simulate_touch_screen {
            ...
            // Keep one pending move. A burst of motion events otherwise
            // makes the next frame walk every widget once per event.
            let events = &mut self.egui_input.events;
            if matches!(events.last(), Some(egui::Event::PointerMoved(_))) {
                events.pop();
            }
